...
async function start() {
    try {
        ...
        console.log("WebSocket server started on IP: "+ip);
        ...
        function sendMessage(user, message) {
            if (user.permissionLevel === 0) {
                ...
                for (let i = 0; i < users.length; i++) users[i].socket.send(JSON.stringify({
                    type: "message",
                    message: "[GUEST] " + user.username + ": " + message
                }));
                ...
                for (let i = 0; i < usersWithPerms.length; i++) {
                    usersWithPerms[i].socket.send(JSON.stringify({
                        type: "message",
                        message: "[MEMBER] " + user.username + ": " + message
                    }));
                }
                ...
                for (let i = 0; i < usersWithPerms.length; i++) {
                    usersWithPerms[i].socket.send(JSON.stringify({
                        type: "message",
                        message: "[ADMIN] " + user.username + ": " + message
                    }));
                }
            }
        }
        ...
        function command(user, message) {
            ...
            if (user.permissionLevel === 2) {
                ...
                if (message.startsWith("/all")) for (let i = 0; i < users.length; i++) users[i].socket.send(JSON.stringify({
                    type: "message",
                    message: "[ALL]" + user.username + ": " + message.replace("/all ", "")
                }));
                else if (message.startsWith("/help")) {
